...
**Generated:** 2026-06-24T12:25:46.109Z
...
| ✅ | Session persists (/api/auth/me) | test_student_1782303946109 |
| ✅ | Login rejects bad password | HTTP 400 |
| ✅ | Logout succeeds | HTTP 200 |
| ❌ | Session cleared after logout | Cookie still valid server-side (HttpOnly cookie persists in node http client) |
...
| ❌ | POST /api/submissions handles unauthenticated | HTTP 201 |
...
- **[Auth] Session cleared after logout** — `Cookie still valid server-side (HttpOnly cookie persists in node http client)`
- **[Submissions] POST /api/submissions handles unauthenticated** — `HTTP 201`
